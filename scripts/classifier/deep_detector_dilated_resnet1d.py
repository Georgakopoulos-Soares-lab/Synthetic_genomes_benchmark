...
"""
deep_detector_dilated_resnet1d.py

Dilated 1D ResNet classifier for distinguishing wild vs synthetic genomic sequences.

Input (Zenodo dataset):
  <DATASET_ROOT>/data/<TAG>/pairs.<TAG>.csv

Each pairs CSV must include columns:
  id, orig, syn
Additional columns are allowed and ignored.

Paths in orig/syn are interpreted as relative to --dataset-root (unless absolute).
For each sequence, the model is trained on N randomly sampled chunks of length L.
Chunk logits are averaged to obtain a sequence-level logit.

Evaluation:
- Leave-one-tag-out cross-validation.
- Checkpoint selection by calibration AUROC.
- Decision threshold chosen to maximize calibration F1, then applied to the held-out tag.
"""
...
def sample_valid_chunk(seq: str, L: int, rng: random.Random, max_tries: int = 50) -> str:
    n = len(seq)
    if n < L:
        # Pad with repetition if seq is non-empty, otherwise pad with random nucleotides
        if n > 0:
            seq = (seq * ((L // n) + 2))[:L]
        else:
            # Empty sequence: pad with random nucleotides
...
def make_collate_fn(L: int, n_chunks: int, seed: int):
    ...
    def collate(batch: List[Dict]):
        ...
        for i, item in enumerate(batch):
            ...
            for _ in range(n_chunks):
                chunk = sample_valid_chunk(seq, L, rng)
                xs.append(one_hot_encode_chunk(chunk))
                gidx.append(i)

        x = torch.stack(xs, dim=0)
        y = torch.tensor(ys, dtype=torch.float32)
        group_index = torch.tensor(gidx, dtype=torch.long)
...
@torch.no_grad()
def collect_scores(model: nn.Module, loader: DataLoader, device: torch.device) -> Tuple[List[int], List[float]]:
    model.eval()
    ys: List[int] = []
    probs: List[float] = []
...
def train_one_length(
    ...
    fit_loader = DataLoader(
        ...
        num_workers=2,
        pin_memory=True,
        collate_fn=make_collate_fn(L, n_train_chunks, seed=seed),
        drop_last=False,
    )
    calib_loader = DataLoader(
        ...
        num_workers=2,
        pin_memory=True,
        collate_fn=make_collate_fn(L, n_eval_chunks, seed=seed + 777),
        drop_last=False,
    )
    test_loader = DataLoader(
        SeqDataset(test_items),
        batch_size=batch,
        shuffle=False,
        num_workers=2,
        pin_memory=True,
        collate_fn=make_collate_fn(L, n_eval_chunks, seed=seed + 999),
        drop_last=False,
    )
    ...
    for ep in range(1, epochs + 1):
        ...
        cal_y, cal_p = collect_scores(model, calib_loader, device)
    ...
    test_y, test_p = collect_scores(model, test_loader, device)
    test_auc = compute_auc(test_y, test_p)
    m = compute_metrics_at_threshold(test_y, test_p, thr)
    ...
    return {
        ...
        "n_fit": int(len(fit_items)),
        "n_calib": int(len(calib_items)),
        "best_calib_auc": float(best_auc),
    }


def run_leave_one_tag_out(
    ...
    for L in lengths:
        ...
        dfL = pd.DataFrame([r for r in per_tag_rows if r["L"] == L])
        if not dfL.empty:
            worst_idx = dfL["auc"].idxmin()
            summary = dict(run_meta)
            summary.update(
                {
                    "L": L,
                    "mean_auc": float(dfL["auc"].mean()),
                    "mean_f1": float(dfL["f1"].mean()),
                    "mean_acc": float(dfL["acc"].mean()),
                    "worst_auc": float(dfL["auc"].min()),
                    "worst_tag": str(dfL.loc[worst_idx, "holdout_tag"]),
                    "n_tags_eval": int(dfL["holdout_tag"].nunique()),
                }
            )
            summary_rows.append(summary)

            print(
                f"\n[L={L}] mean AUROC={dfL['auc'].mean():.4f} | mean F1={dfL['f1'].mean():.4f} "
                f"| worst AUROC={dfL['auc'].min():.4f} ({dfL.loc[worst_idx, 'holdout_tag']})"
            )
    ...
    df_per_tag.to_csv(out_path.with_name(out_path.stem + "_per_tag.csv"), index=False)

    print(f"[ok] wrote summary: {out_path}")
    print(f"[ok] wrote per-tag: {out_path.with_name(out_path.stem + '_per_tag.csv')}")
...
def parse_args():
    ...
    ap.add_argument("--tags", nargs="+", required=True, help="Tag names (must match dataset folder names).")
    ap.add_argument("--lengths", nargs="+", type=int, required=True, help="Chunk lengths (bp) to sweep.")
    ...
    ap.add_argument("--n-eval-chunks", type=int, default=32)
    ...
    ap.add_argument("--eval", choices=["leave_one_tag_out"], default="leave_one_tag_out")
    ap.add_argument("--seed", type=int, default=7)
    ap.add_argument("--out", required=True, help="Output CSV path for summary.")
...
def main():
    ...
    for tag in args.tags:
        ...
        labs = set(int(it["label"]) for it in items)
        if labs != {0, 1}:
            print(f"[warn] tag={tag} labels present={labs} (expected both 0 and 1).")
    ...
    run_meta = {
        ...
        "n_eval_chunks": int(args.n_eval_chunks),
        ...
        "dilations": json.dumps(list(dil)),
        "seed": int(args.seed),
    }

    out_path = Path(args.out)
    if args.eval == "leave_one_tag_out":
        df = run_leave_one_tag_out(
            ...
            n_train_chunks=args.n_train_chunks,
        ...
        raise SystemExit("Only leave_one_tag_out is implemented.")
